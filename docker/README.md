...
(preferably) or docker or macOS with Apple's [containerization](https://github.com/apple/containerization).
...
Make targets:
  ...
  * `make <engine>-sh`: build and drop into bash in the build container
...
Set `DOCKER_ARCH` environment variable to `amd64` or `arm64` explicitly.
Should work out of the box with macOS containerization; on Linux, podman
needs qemu binaries.

```sh
$ sudo apt install qemu-user-static
$ rm -rf ../.cache/iid               # maybe clear make cache
$ DOCKER_ARCH=amd64 make
```

Other architectures supported by debian/docker/qemu (e.g. `riscv64`) may
somewhat work too, but not tested and polished.  Adjust configs to use
debian's clang19 for builds, as apt.llvm.org's packages for newer versions
have a more limited set of architectures.

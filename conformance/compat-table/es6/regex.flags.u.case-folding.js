// compat-table: ES6 > syntax > RegExp "y" and "u" flags (medium) > "u" flag, case folding
// mdn: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp#Parameters
// spec: http://www.ecma-international.org/ecma-262/6.0/#sec-get-regexp.prototype.sticky
//
// SPDX-FileCopyrightText: 2010-2013 Juriy Zaytsev
// SPDX-FileCopyrightText: 2025 Ivan Krasilnikov
// SPDX-License-Identifier: MIT

function testCode() {
  return "ſ".match(/S/iu) && "S".match(/ſ/iu);
}

try {
  if (testCode()) {
    console.log("compat-table/es6/regex.flags.u.case-folding.js: OK");
  } else {
    console.log("compat-table/es6/regex.flags.u.case-folding.js: failed");
  }
} catch (e) {
  console.log("compat-table/es6/regex.flags.u.case-folding.js: exception: " + e);
}

// compat-table: ES6 > built-ins > Reflect (small) > Reflect.isExtensible
// mdn: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Reflect/isExtensible
// spec: http://www.ecma-international.org/ecma-262/6.0/#sec-reflection
//
// SPDX-FileCopyrightText: 2010-2013 Juriy Zaytsev
// SPDX-FileCopyrightText: 2025 Ivan Krasilnikov
// SPDX-License-Identifier: MIT

function testCode() {
  return Reflect.isExtensible({}) &&
    !Reflect.isExtensible(Object.preventExtensions({}));
}

try {
  if (testCode()) {
    console.log("compat-table/es6/Reflect.isExtensible.js: OK");
  } else {
    console.log("compat-table/es6/Reflect.isExtensible.js: failed");
  }
} catch (e) {
  console.log("compat-table/es6/Reflect.isExtensible.js: exception: " + e);
}

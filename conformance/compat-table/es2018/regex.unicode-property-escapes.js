// compat-table: ES2016+ > 2018 features > RegExp Unicode Property Escapes (small) > basic
// mdn: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions/Unicode_Property_Escapes
// spec: https://github.com/tc39/proposal-regexp-unicode-property-escapes
//
// SPDX-FileCopyrightText: 2010-2013 Juriy Zaytsev
// SPDX-FileCopyrightText: 2025 Ivan Krasilnikov
// SPDX-License-Identifier: MIT

function testCode() {
  const regexGreekSymbol = /\p{Script=Greek}/u;
  return regexGreekSymbol.test('π');
}

try {
  if (testCode()) {
    console.log("compat-table/es2018/regex.unicode-property-escapes.js: OK");
  } else {
    console.log("compat-table/es2018/regex.unicode-property-escapes.js: failed");
  }
} catch (e) {
  console.log("compat-table/es2018/regex.unicode-property-escapes.js: exception: " + e);
}

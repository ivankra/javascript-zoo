// compat-table: ES2016+ > 2025 features > Iterator Helpers (large) > Iterator.from, iterator
// mdn: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Iterator/from
// spec: https://github.com/tc39/proposal-iterator-helpers
//
// SPDX-FileCopyrightText: 2010-2013 Juriy Zaytsev
// SPDX-FileCopyrightText: 2025 Ivan Krasilnikov
// SPDX-License-Identifier: MIT

function testCode() {
  const iterator = Iterator.from({
    i: 0,
    next() {
      return { value: ++this.i, done: this.i > 3 };
    }
  });
  return 'next' in iterator
    && iterator instanceof Iterator
    && Array.from(iterator).join() === '1,2,3';
}

try {
  if (testCode()) {
    console.log("compat-table/es2025/Iterator.from.iterator.js: OK");
  } else {
    console.log("compat-table/es2025/Iterator.from.iterator.js: failed");
  }
} catch (e) {
  console.log("compat-table/es2025/Iterator.from.iterator.js: exception: " + e);
}
